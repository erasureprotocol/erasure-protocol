...
async function createInstance(name, calldata) {
  let createAdddress, createSaltyAdddress
...
const main = async () => {
  ...
  console.log(`userAddress: ${deploymentWallet.address}`)
  console.log(`proofhash: ${proofhash}`)
  console.log(`IPFShash: ${IPFShash}`)
  console.log(``)

  // Feed
  const [instance1, instance2] = await createInstance(
    'Feed',
    abiEncodeWithSelector(
      'initialize',
      ['address', 'bytes32', 'bytes'],
      [deploymentWallet.address, proofhash, IPFShash],
    ),
  )
  ...
  async function submitHash(address, proofhash) {
    ...
    await c.Feed.instance.submitHash(proofhash).then(async txn => {
      const receipt = await provider.getTransactionReceipt(txn.hash)
